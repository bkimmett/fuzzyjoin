...
import ca.uvic.csc.research.ByteArrayWritable;
...
public class FuzzyJoinNaiveBin {
	...
	//
  	...
  	public static class FuzzyJoinNaiveBinMapper 
       extends Mapper<NullWritable, IntWritable, ByteArrayWritable, MetadataIntWritable>{
    ...
    private MetadataIntWritable valueOut;
    ...
    public void map(NullWritable key, IntWritable value, Context context
     ...
     // String[] line = value.toString().trim().split("\\s",1);
      ...
      key1 = new ByteWritable((byte)Math.abs(value.get() % granularity_mod)); // parse as binary, modulo appropriately
      valueOut = new MetadataIntWritable(value.get());
  ...
  public static class TriangleHashPartitioner extends Partitioner<ByteArrayWritable, MetadataIntWritable>
	{
	   @Override
 	   public int getPartition(ByteArrayWritable key, MetadataIntWritable value,int numReduceTasks)
	 ...
	 public static class FuzzyJoinNaiveNullReducer
       extends Reducer<ByteArrayWritable,MetadataIntWritable,Text,Text> {
  	//dummy - reduces nothing. Used for measuring comm. cost
  		public void reduce(ByteArrayWritable key, Iterable<MetadataIntWritable> values, 
                       Context context
                       ) throws IOException, InterruptedException {
                       return;
        }
  ...
  public static class FuzzyJoinNaiveBinReducer
       extends Reducer<ByteArrayWritable,MetadataIntWritable,Text,Text> {
    ...
    public void reduce(ByteArrayWritable key, Iterable<MetadataIntWritable> values, 
        ...
        boolean hybrid_mode = true; //starts assuming it's getting in two sets of items and joining just those
     	...
     	ArrayList<Integer> int_list_1 = new ArrayList<Integer>();
     	ArrayList<Integer> int_list_2 = new ArrayList<Integer>();
     	...
     	for (MetadataIntWritable val : values) { // no need to keep a dict; the system tosses by key!!
     		...
     		if(!hybrid_mode || dir == 0){
     			int_list_1.add(val.get());
     			hybrid_mode = false;
     		} else if (dir == 1) {
     			int_list_1.add(val.get());
     		} else if (dir == 2) {
     			int_list_2.add(val.get());
     		}
       	}
      ...
      //that's all data from the reducer read in. Next is processing it.
     	 ...
     	 int list_1_size = int_list_1.size();
     	 int list_2_size = int_list_2.size();
		...
		context.setStatus("reduce: joining data [two sets]");
			for(int i=0; i< list_1_size; i++){
				...
				for(int j=0; j< list_2_size; j++){
					if(compareDistance(int_list_1.get(i),int_list_2.get(j)) <= distance_threshold){
						if(!null_absolute){
							context.write(new Text(String.format("%32s", Integer.toBinaryString(int_list_1.get(i))).replace(" ", "0")), new Text(String.format("%32s", Integer.toBinaryString(int_list_2.get(j))).replace(" ", "0"))); // perform comparison. If a success, kick out the pair
						} else {
							records_written++;
						}
					}
			...
			for(int i=0; i< list_1_size; i++){
				...
				for(int j=i+1; j< list_1_size; j++){
					if(compareDistance(int_list_1.get(i),int_list_1.get(j)) <= distance_threshold){
						if(!null_absolute){
							context.write(new Text(String.format("%32s", Integer.toBinaryString(int_list_1.get(i))).replace(" ", "0")), new Text(String.format("%32s", Integer.toBinaryString(int_list_1.get(j))).replace(" ", "0"))); // perform comparison. If a success, kick out the pair
						} else {
							records_written++;
						}
					}
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (otherArgs.length !=4) {
      System.err.println("Usage: FuzzyJoinNaiveBin <in> <out> <comparison_threshold> <granularity>; granularity must be less than or equal to 2^(input string length). Setting <out> to 'null' discards output; 'null-cost' discards after copying to reducer.");
      System.exit(2);
    }
    ...
    job.setOutputValueClass(Text.class);
   	...
   	if(otherArgs[1].equals("null")){
		...
		job.setReducerClass(FuzzyJoinNaiveBinReducer.class);
